...
const App = () =>  {
  ...
  const addDocument = (index) => {
      if (selectedFile) {
          ...
          updatedApplicants[index].documents.push({ name: selectedFile.name, label: `Document ${updatedApplicants[index].documents.length + 1}` });
        ...
        <div className="applicant-card shadow-box">
            ...
            <ul className="mt-3 document-list">
                {applicants[currentApplicantIndex].documents.map((doc, i) => (
                    <li key={i}><strong>{doc.label}:</strong> {doc.name}</li>
                ))}
            </ul>
